...
// busy_timeout is 0, and even *opening* the connection or switching
// journal modes can throw SQLITE_BUSY/"database is locked" if another
// process holds a lock at that exact instant — this crashed a real
// Railway build ("Failed to collect page data for /api/scan/[id]") on
// exactly that class of error. So the connection open, pragmas, and full
// schema setup all happen inside one retry loop below: on any lock-shaped
// error, sleep synchronously (better-sqlite3 is sync, and this all runs at
// module load before the server accepts requests) and redo the whole
// thing from scratch. Safe to repeat — CREATE TABLE/INDEX IF NOT EXISTS
// and addColumnIfMissing (which tolerates "duplicate column name" itself)
// are all naturally idempotent, so a retried run just re-confirms what's
// already there.
...
function withLockRetry<T>(fn: () => T, attempts = 10): T {
  let lastErr: unknown;
...
// second process racing to add the same column at the same time. SQLite
// has no "ADD COLUMN IF NOT EXISTS", so without this, whichever process
// loses that race throws. Any other error still throws.
...
function initSchema(db: Database.Database): void {
  ...
  // working unchanged.
  addColumnIfMissing(db, "bulk_scans", "universe_id", `universe_id TEXT REFERENCES universes(id)`);
  ...
  // not a sub-vertical.
  addColumnIfMissing(db, "bulk_scan_topics", "category", `category TEXT`);
  addColumnIfMissing(db, "universe_topics", "category", `category TEXT`);
  ...
  addColumnIfMissing(db, "bulk_scans", "prompt_mode", `prompt_mode TEXT NOT NULL DEFAULT 'question'`);
  ...
  addColumnIfMissing(db, "universes", "user_id", `user_id TEXT REFERENCES users(id)`);
  db.exec(`CREATE INDEX IF NOT EXISTS idx_universes_user ON universes(user_id);`);
  addColumnIfMissing(db, "bulk_scans", "user_id", `user_id TEXT REFERENCES users(id)`);
  ...
  addColumnIfMissing(db, "universes", "categorization_status", `categorization_status TEXT`); // pending|running|complete|error, NULL for pre-existing small universes
  addColumnIfMissing(db, "universes", "categorization_error", `categorization_error TEXT`);
  ...
  addColumnIfMissing(db, "universes", "theme_list", `theme_list TEXT`);
  addColumnIfMissing(db, "universes", "tracked_themes", `tracked_themes TEXT NOT NULL DEFAULT '[]'`); // JSON array of opted-in category values
  addColumnIfMissing(db, "universes", "auto_run_enabled", `auto_run_enabled INTEGER NOT NULL DEFAULT 0`);
  addColumnIfMissing(db, "universes", "last_auto_run_at", `last_auto_run_at TEXT`);
  // 1 = one of the top-N-by-volume keywords in a tracked theme; this is
  // exactly the set startUniverseRun() scans.
  addColumnIfMissing(db, "universe_topics", "tracked", `tracked INTEGER NOT NULL DEFAULT 0`);
...
function openAndInit(): Database.Database {
  ...
  try {
    ...
    instance.close(); // don't leak a handle per retry attempt
    throw err;
  }
}

const db = withLockRetry(openAndInit);
